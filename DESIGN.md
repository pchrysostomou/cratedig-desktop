...
| **4 — Library + player** | `/stream/{id}` (`FileResponse` 206) + `/cover/{id}`; `useAudioEngine` + `PlayerBar`; play/pause/seek/volume; `ended`→next; `/history`. | Click a track → it plays **and seeks**; queue auto-advances; play count increments. |
